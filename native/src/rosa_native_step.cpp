...
class NativeState {
 public:
  explicit NativeState(py::object state) : state_(std::move(state)) {
    ...
    if (abi != 1) throw py::value_error("unsupported native state ABI");
  ...
  py::array_t<int64_t> step(
      py::array_t<int64_t, py::array::c_style | py::array::forcecast> tokens) {
    ...
    int64_t* out = output.mutable_data();
    {
      ...
      for (int64_t b = 0; b < batch_; ++b) out[b] = step_row(b, in[b]);
    }
    ++position_;
 ...
 private:
  ...
  void validate_shapes() {
    if (batch_ <= 0 || max_length_ <= 0 || position_ < 0 ||
        position_ > max_length_ || state_capacity_ <= 0 || edge_capacity_ <= 0 ||
        hash_capacity_ <= 0) {
  ...
  int32_t find_transition(int64_t b, int32_t state, int64_t token) const {
    ...
    while (hs[idx(b, hash_capacity_, slot)] != -1) {
      ...
      if (hs[at] == state && ht[at] == token) return he[at];
  ...
  int32_t add_transition(int64_t b, int32_t count, int32_t state,
    ...
    if (count >= edge_capacity_) throw std::runtime_error("transition capacity exceeded");
  ...
  void replace_transition(int64_t b, int32_t state, int64_t token,
    ...
    if (edge == -1) throw std::runtime_error("transition not found");
  ...
  void path_assign(int64_t b, int32_t node, int64_t assigned) {
    access(b, node); apply(b, node, assigned);
  }
  ...
  int64_t step_row(int64_t b, int64_t token) {
    ...
    if (state == -1) {
      ...
      if (length_.data()[idx(b, state_capacity_, state)] + 1 ==
        ...
        value_.mutable_data()[idx(b, state_capacity_, clone)] = point_query(b, target);
        int32_t edge = head_.data()[idx(b, state_capacity_, target)];
        while (edge != -1) {
          edge_count = add_transition(
              b, edge_count, clone, edge_token_.data()[idx(b, edge_capacity_, edge)],
    ...
    last = current;
    const int32_t matched = suffix_link_.data()[idx(b, state_capacity_, current)];
    int64_t source = -1;
    if (matched != 0) source = point_query(b, matched);
    int64_t prediction = -1;
    if (source >= 0) prediction = history_.data()[idx(b, max_length_, source + 1)];
    path_assign(b, current, position_);
    last_a[b] = last; size_a[b] = size; edge_count_a[b] = edge_count;
    return prediction;
  }
  ...
  py::array_t<int32_t, py::array::c_style> head_, edge_target_, edge_next_,
      hash_state_, hash_edge_, suffix_link_, length_, left_, right_, parent_, stack_,
      last_, size_, edge_count_;
  py::array_t<uint8_t, py::array::c_style> lazy_valid_;
  int64_t batch_, max_length_, position_, state_capacity_, edge_capacity_, hash_capacity_;
};
